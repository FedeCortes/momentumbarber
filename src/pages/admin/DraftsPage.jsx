      ...
      <span className="text-cream/60 min-w-0">
        ...
        {show && (
          <span className="ml-1.5 align-middle inline-block">
            <CommissionBadge pct={pct} size="xs"
              isDefault={it.item_type === 'service' && pct === Number(barber?.commission_pct)}
...
function DraftRow({ draft, barbers, paymentMethods, barberSvcs, barberPrds, onChange, showDate, isAdmin, onDelete, compact }) {
  ...
  async function copyToOfficial() {
    ...
    try {
      ...
      // Se respeta el % que quedó guardado en cada ítem del borrador (servicios y productos por separado)
      const svcCommissionAmt = barber
        ...
        <div className="flex flex-col gap-5">
          ...
          {catalog.products.length > 0 && (
            <div>
              ...
              <EditItemPicker items={catalog.products} selected={selProducts} onToggle={(it, d) => toggleItem(setSelProducts, it, d)} commissionOf={eProductCommissionOf} />
            </div>
          )}
        ...
        <div className="flex flex-col gap-5">
          ...
          {catalog.products.length > 0 && (
            <div>
              ...
              <EditItemPicker items={catalog.products} selected={selProducts} onToggle={(it, d) => toggleItem(setSelProducts, it, d)} commissionOf={sProductCommissionOf} />
            </div>
          )}
